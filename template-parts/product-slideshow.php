<?php
/**
 * Template part for displaying slideshows on product pages
 *
    ...
    global $post;
     ...
     if ( $featured_images ):
        ?>
            ...
            $largeSizedImage = $dynamic_featured_image->get_image_url($images['attachment_id'], 'amv-slider-image'); 
...
if (has_post_thumbnail):
        echo '<li>';
        the_post_thumbnail( 'amv-slider-thumb' );
        echo '<p class="flex-caption">';
    $full_image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full');
    echo '<a href="' . $full_image_url[0] . '" title="' . the_title_attribute('echo=0') . '" ';?>
     target="_blank"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span> Download Hi-Res</a></p>
...
if( class_exists('Dynamic_Featured_Image') ):
    ...
    global $post;
     ...
     if ( $featured_images ):
        ?>
            <?php foreach( $featured_images as $images ): 
              $largeSizedImage = $dynamic_featured_image->get_image_url($images['attachment_id'], 'amv-slider-thumb'); ?>
